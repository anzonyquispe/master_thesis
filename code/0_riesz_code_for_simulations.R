...
get_stage1<-function(args,type,Y,X,p0,D_LB,D_add,max_iter,b,alpha_estimator = 1,gamma_estimator = 1){
  ...
  t.coef <- glmnet(X_const, args$T, family="binomial", alpha=1, lambda = t.coef$lambda.min )
  prob.t<-function(d,z){
...
##################
  ...
  beta_dml <- results[5]
  se_dml <- results[6]
  result_final <- c(beta_adml, se_adml, beta_dml, se_dml)
