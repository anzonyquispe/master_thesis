...
dml_dr <- function(dat,est='ols',p=2, K = 5) {
  ...
  for (k in 1:K) {
    ...
    if (est == "ols"){
      ...
      # Split the data
      # Split the data
      train <- dat[-idx, ]   # new values to predict
      test <- dat[idx, ]   # new values to predict
      
      # Fit N-W regression on training data
      nw_model <- npreg(D ~ X, data = train, regtype = "lc", bwmethod = "cv.aic")
...
generate_data_binary <- function(n = 200, 
                                 a0 = 0.5, 
                                 a1 = 0.3, 
                                 s1 = 1, 
                                 alpha = 1, 
  ...
  # Generate covariates x1 and x3
  x1 <- runif(n, 0, 1)  # Uniform[0, 1] for x(:,1)
  x3 <- runif(n, 0, 1)  # Uniform[0, 1] for x(:,3)
  ...
  # Generate binary d using logit or probit
  r <- if (model == "probit") pnorm(index) else 1 / (1 + exp(-index))
  d <- as.integer(r > runif(n))  # Compare to uniform random variable
  
  # Generate y
  y <- alpha * d + b0 * exp(x1) / (1 + exp(x1)) + b1 * x3 + s2 * rnorm(n)
  
  # Return data as a data frame
  data.frame(x1 = x1, x3 = x3, d = d, y = y, r = r)
}
